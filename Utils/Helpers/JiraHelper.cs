                        ...
                        if (null != workLog.Author && !string.IsNullOrWhiteSpace(workLog.Author.EmailAddress) && workLog.Started.HasValue)
                        {
                            ...
                            else
                            {
                                ...
                                if (null == rtm)
                                {
                                    ...
                                    {
                                        TeamMemberName = memberName, TeamMemberEmail = memberEmail,
                                        Concerned = false
                                    };
                            ...
                            if (workLog.TimeSpentSeconds.HasValue)
                            {
                                ...
                                if (null == rw)
                                {
                                    ...
                                    {
                                        WorklogJiraId = workLog.Id, TeamMemberId = memberId,
                                        WorklogDate = started.Date, TimeSpentSeconds = workLog.TimeSpentSeconds.Value
                                    });
                                    db.SaveChanges();
                                }
                            }
