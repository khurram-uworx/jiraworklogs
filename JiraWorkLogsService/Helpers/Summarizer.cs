...
class Summarizer
{
    ...
    public async Task<int> ProcessAsync(string[] emails, DateTime start, JiraDbContext db)
    {
    ...
    public async Task<int> ProcessAsync(string[] emails, JiraDbContext db)
    {
        var previousMonth = DateTime.Now.AddMonths(-1);
        var start = new DateTime(previousMonth.Year, previousMonth.Month, 1);
        return await ProcessAsync(emails, start, db);
    }
}
